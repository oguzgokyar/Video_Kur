...
class SocialMediaScheduler:
    ...
    def __init__(self, base_dir: str, check_interval: int = 60):
        ...
        print(f"[SOCIAL] ℹ️  Artık queues.json kullanılıyor (social_queue.json deprecated)")
    ...
    def _reschedule_to_next_day(self, item: Dict, queue: Dict, queues_data: dict) -> Optional[str]:
        ...
        try:
            ...
            # Update social_queue.json
            social_queue_file = self.data_dir / 'social_queue.json'
            if social_queue_file.exists():
                with open(social_queue_file, 'r', encoding='utf-8') as f:
                    social_data = json.load(f)
                
                for sq_item in social_data.get('queue', []):
                    if sq_item.get('queue_id') == item.get('queue_id'):
                        sq_item['scheduled_time'] = new_scheduled_str
                        sq_item['rescheduled_from'] = item.get('scheduled_time', '')
                        sq_item['reschedule_reason'] = 'daily_limit_reached'
                        break
                
                with open(social_queue_file, 'w', encoding='utf-8') as f:
                    json.dump(social_data, f, ensure_ascii=False, indent=2)
            
            # Also update queues.json video entry with scheduled_publish_date
    ...
    def _count_today_uploads(self, queue_id: str) -> int:
        ...
        """
        # Check social_queue.json for completed uploads today
        social_queue_file = self.data_dir / 'social_queue.json'
        ...
        try:
            ...
            for sq_item in social_data.get('queue', []):
                ...
                item_queue_id = sq_item.get('queue_id', '')
                if not queue_id in item_queue_id:
                    continue
                
                # Check if completed today
                if sq_item.get('status') == 'success':
                    completed_at = sq_item.get('completed_at', '')
                    if completed_at:
                        try:
